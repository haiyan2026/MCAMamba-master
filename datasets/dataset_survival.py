...
class Generic_WSI_Survival_Dataset(Dataset):
    def __init__(self, gene_dir, csv_path, modal='omic', apply_sig=False,
        ...
        if self.apply_sig:
            self.signatures = pd.read_csv(self.gene_dir)
        else:
    ...
    def get_split_from_df(self, all_splits: dict, split_key: str = 'train', scaler=None):
        ...
        if len(split) > 0:
            ...
            split = Generic_Split(df_slice, gene_dir = self.gene_dir, metadata=self.metadata, modal=self.modal, signatures=self.signatures,
            ...
            if self.data_dir:
                if self.modal == 'path':
                    ...
                    for i in range(self.num_pathways):
                        omic_list.append(torch.tensor(self.genomic_features[self.omic_names[i]].iloc[idx]))
                    return (path_features, omic_list, label, event_time, c, case_id)
...
class Generic_Split(Generic_MIL_Survival_Dataset):
    def __init__(self, slide_data, metadata, modal, gene_dir, signatures=None, data_dir=None, label_col=None, patient_dict=None, num_classes=2):
        self.use_h5 = False
        self.slide_data = slide_data
        ...
        if self.signatures is not None:
            ...
            self.num_pathways = len(self.omic_names)
